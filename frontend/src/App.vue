...
function switchNav(key) { active.value = key; if (key === 'builds') refreshBuilds(); if (key === 'imageFactory') refreshImageExports() }
  ...
  <div class="shell">
    ...
    <main class="main">
      ...
      <section v-else-if="active === 'projects'" class="content">
        <div class="section-actions"><div><h2>项目</h2><p>先固定 x86 / ARM 架构，再绑定前后端 Git 仓库和对应提交的应用镜像。</p></div><button class="primary" @click="openProjectForm()">＋ 新建项目</button></div>
        <div class="workspace-grid"><div class="list-panel"><button v-for="project in projects" :key="project.id" class="project-item" :class="{selected:selectedProjectId===project.id}" @click="selectedProjectId=project.id"><span class="project-logo">{{ project.name.slice(0,1).toUpperCase() }}</span><span><b>{{ project.name }}</b><small>{{ project.appKey }} · {{ project.targetArch }} · {{ project.currentVersion || '未设置版本' }}</small></span><em>›</em></button><div v-if="!projects.length" class="empty">还没有项目</div></div>
          <div v-if="selectedProject" class="detail-panel"><div class="detail-title"><div><span class="project-logo large">{{ selectedProject.name.slice(0,1) }}</span><div><h2>{{ selectedProject.name }}</h2><p>{{ selectedProject.description || '暂无描述' }}</p></div></div><div><button class="ghost" @click="openProjectForm(selectedProject)">编辑</button><button class="danger-link" @click="removeProject(selectedProject)">删除</button></div></div>
            ...
            <div class="form-section"><b>源码仓库与应用镜像</b><small>上传的镜像 TAR 必须绑定仓库中的 commit id</small></div>
            <div class="profile-grid"><article v-for="role in ['BACKEND','FRONTEND']" :key="role" class="profile-card"><div class="profile-top"><span>{{ role === 'BACKEND' ? 'B' : 'F' }}</span><div><h3>{{ role === 'BACKEND' ? '后端' : '前端' }}</h3><p v-if="repositoryFor(selectedProject, role)" class="mono">{{ repositoryFor(selectedProject, role).url }}</p><p v-else>尚未绑定 Git 仓库</p></div><button @click="openRepositoryForm(role)">{{ repositoryFor(selectedProject, role) ? '编辑仓库' : '绑定仓库' }}</button></div><div class="profile-services"><div v-for="a in projectApplicationArtifacts(selectedProject.id, role).slice(0,4)" :key="a.id"><b>v{{ a.version }} · {{ short(a.gitCommit,10) }}</b><small>{{ formatDate(a.createdAt) }}</small></div><div v-if="!projectApplicationArtifacts(selectedProject.id, role).length"><b>暂无镜像</b><small>请先在外部构建并 docker save</small></div></div><footer><span>{{ repositoryFor(selectedProject, role)?.ref || '未设置分支' }}</span><button class="text-btn" @click="openApplicationForm(role)">＋ 上传镜像 TAR</button></footer></article></div>
