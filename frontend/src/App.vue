...
const artifactForm = reactive({ component: 'docker-engine', version: '', sourcePath: '', arch: 'amd64' })
...
const requiredArtifacts = computed(() => {
  ...
  return ['docker-engine', 'docker-compose', ...mw]
})
const INFRA = [{ component: 'docker-engine', displayName: 'Docker Engine' }, { component: 'docker-compose', displayName: 'Docker Compose' }]
...
async function loadAll(silent = false) {
  ...
  try {
    const [d, s, p, c, a, b, cat] = await Promise.all([api.get('/dashboard'), api.get('/system'), api.get('/projects'), api.get('/profiles'), api.get('/artifacts'), api.get('/builds'), api.get('/middleware/catalog')])
